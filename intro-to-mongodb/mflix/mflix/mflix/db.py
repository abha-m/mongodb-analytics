...
try: 
    db = MongoClient("mongodb+srv://analytics:analytics-password@mflix-nfkcg.mongodb.net/test?retryWrites=true&w=majority")["mflix"]
except KeyError:
...
def get_movie(id):
    try:
        return db.movies.find_one({"_id": ObjectId(id)})
    except InvalidId:
        return None
...
def delete_comment_from_movie(movieid, commentid):
    ...
    movie = db.movies.find_one({"comments._id": commentid})
